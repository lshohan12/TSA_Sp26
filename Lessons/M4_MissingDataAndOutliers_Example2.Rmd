...
```
> can see the boxplots come from a lognormal distribution???
...
#detrend deseason series before grubs test
...
cat(mean_inflow,sd_inflow)
# mean = NA NA-4.839207 764.0217 ----> 

```

decompose - seasonaal component 
detrend linear model - subtracting from data 
...
sum(is.na(df_inflow_random$inflow)). #ways to check NAs in specific column
...
ggplot(df_inflow_random, aes(y=inflow)) +
            geom_boxplot() 
```
> boxplot 1 - what we are seeing for some of the inflow series is that the wet season high values persist on hihg months when it should return to normal ... 
...
df_inflow_random$inflow <- rm.outlier(df_inflow_random$inflow,fill=TRUE) #using fill equal true the value will be replaced by mean or median 
      # = true means you want to replace it with the mean of the series ...
...
#.... asnwer
#both tests confirm highest observation is an outlier - decide you want to remove that? next step is rm.outlier()
#find outlier, then do tests to confirm, then remove it and generate plot and over and over again - need a loop to do it because when you submit this itll run code chunk just once... this is how tsclean function was created (basically a loop that continues going through all the values)
