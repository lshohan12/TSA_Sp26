...
#ggplot(inflow_data, aes(x=my_date,y=HP3)) + G
...
#dont throw entire dataframe into ts function - first column is time stamp dont wanna convert date into a TS 
#start - want ts to know when to start and specify time of first observation - specify first year and first month - can use first month first year from lubridate package - #year, month, day but dont have day
#frequency - when observations start repeating themselves - monthly data start repeating after 12 observations - monthly is 12; daily is 365; - only need with seasonal 
...
```{r}
#dont need to specify aesthetics with autoplot 
...
```{r acf}
# USE UPPERCASE FUNCTION DONT USE LOWERCASE - HARD TO DESCRIBE AND ANALYZE

# Option 1: The function acf() and pacf() is from package "stats"
HP1_acf= acf(ts_inflow_data[,1], lag.max = 60) 
        #lag.max = how many lags i want to plot = 40 standard but can do more to see decay over time 
        #plot = TRUE is default -> ACF plot will show up on window 
      #any lags within blue line window are insiginificant and count as zero and have no correlation 


HP1_pacf= pacf(ts_inflow_data[,1], lag.max = 60) 
...
HP1_acf=Acf(ts_inflow_data[,1],lag.max = 60, plot=TRUE)
    #arguements the same but plots different
    #omits lag 0 in the plot

HP1_acf=Acf(ts_inflow_data[,2])
HP1_acf=Acf(ts_inflow_data[,3])
    #negative lags not as significant as 1


HP1_pacf= Pacf(ts_inflow_data[,1], lag.max = 60, plot=TRUE)
    # 2nd bar flips to negative shows the influence of t-1 that should be negative is postive in acf func


#ANTOHER EXAMPLES OF HP4 
#all positive acf , you still have some seasonality happening, could be trend component is so strong that its shading seasonality - start sepearting two components 
#less dependent with later on lags and less memory 
