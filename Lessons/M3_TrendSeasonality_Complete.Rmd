...
raw_inflow_data <- read.table(file="/home/guest/TSA_Sp26/Data/inflowtimeseries.txt",header=FALSE,skip=0)
...
```{r}
par(mfrow=c(1,2))  #place plot side by side
Acf(ts_inflow_data[,i],lag.max=40,main=paste("Inflows HP",iHP,sep="")) 
Acf(ts_detrend_inflow_data,lag.max=40,main=paste("Inflows HP",iHP,sep="")) 


Pacf(ts_inflow_data[,i],lag.max=40,main=paste("Inflows HP",iHP,sep=""))
Pacf(ts_detrend_inflow_data,lag.max=40,main=paste("Inflows HP",iHP,sep=""))
...
answer is noo you should detrend time trend first, and then eliminate seasonality from data set ... 
...
```
on four - yearly averages have a stronger trend than hp1
...
```
result of= 2-sided pvalue =0.010364
- rejecting null and saying there is a trend in my data set

...result for mann kendall and smk test both lead us to reject null...

compare yearly average there is no trend but look at season behavior there are trend within seasonal trend behavior??
- smk.test = more detail on statistics in datasets


---- on 4 -----
on season - all S have positive vallues so seeing a trend
pvalue for mankendall is very low which means that we reject null and say that series has deterministic trend 
...
```
p-value = 0.01 --> reject null hypo that series has a unit root/stochastic trend
    - saying no but we see a deterministic trend with the other ones 
    
yearly has similar results 


-----hp = 4 ----
pvalue of 0.01 
- rject null that data has unit root so thus we have a trend but based on mankendall is deterministic and not stochastic 

but for yearly data --> got 0.15 - when doing time series model we run it on monthly not yearly, we go with test results you are building model for so aka monthly not yearly 
