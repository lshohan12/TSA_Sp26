...
```{r}
#2025 is not a complete year so i decided to trim it out to do yearly aggregation, 
#this will produce similar results if i had also kept those few months

ts_energydata_trimmed <- window(ts_energydata, end=c(2024, 12))

#Group data in yearly steps instances
energy_data_matrix <- matrix(ts_energydata_trimmed,byrow=FALSE,nrow=12)
energy_data_yearly <- colMeans(energy_data_matrix)


energy_data_new_yearly <- ts(energy_data_yearly, start = 1973, frequency = 1)
...
The ADF test of dickey-fuller of -0.853 and pvalue of 0.9515 we fail to reject the null hypothesis and thus this series having a unit root we can conclude the yearly series is non stationary and has a stochastic trend which is what we determined from Q5. 
